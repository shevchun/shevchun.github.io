...
var _custom = __webpack_require__(28);
...
var _facebook = __webpack_require__(20);
...
var _odnoklassniki = __webpack_require__(21);
...
var _plusone = __webpack_require__(23);
...
var _linkedin = __webpack_require__(27);
...
exports.default = function () {
  ...
  if ($('.sources__head-btn').length) {
    $('.sources__head-btn').on('click', function () {
      $(this).parents('.sources').toggleClass('active');
      $(this).parents('.sources').find('.sources__body').slideToggle();
    });
  }

  if ($('.blog-product__mobile-btn').length) {
    $('.blog-product__mobile-btn').on('click', function () {
      $(this).siblings('.blog-product__text').toggleClass('active');
  ...
  if ($('.js-nav').length) {
    ...
    $('.js-nav a').on('click', function ancordsLinks(e) {
  ...
  $('.review2__item-btn').on('click', function () {
    $(this).parents('.review2__item').toggleClass('active');
  });

  $('.star-rating__item').on('click', function () {
    var parent = $(this).parent('.star-rating'),
        rating = $(this).data('value'),
        valueHolder = parent.find('.star-rating__value');

    if (!parent.hasClass('disabled')) {
      $(this).addClass('active');
      $(this).prevAll('.star-rating__item').addClass('active');
      $(this).nextAll('.star-rating__item').removeClass('active');
      valueHolder.val(rating).change();
      valueHolder.attr('value', rating);
    };
  });
  $('.star-rating__value').on('change', function () {
  ...
  $(".js-input").each(function (index, element) {
    if ($(element).val() !== "") {
      $(element).parents(".js-input-parent").addClass('active');
    }
  });
  ...
  $(window).on('load resize', function () {
    if ($(window).width() < 767) {
      $('.blog-product').each(function (index, element) {
        $(element).find('.blog-product__title').insertBefore($(element).find('.blog-product__head'));
        $(element).find('.blog-product__btn-box:not(.blog-product__btn-box--no-js)').insertBefore($(element).find('.blog-product__body'));
      });
      $('.cart__item').each(function (index, element) {
        $(element).find('.cart__item-cell').append($(element).find('.cart__item-count'));
      });
      if ($('.product-page__controls').length) {
        $('body').addClass('pb');
      }
      if ($('.checkout__pay-btn-box').length) {
        $('body').addClass('pb2');
      }
    } else {
      $('.blog-product').each(function (index, element) {
        $(element).find('.blog-product__title').insertBefore($(element).find('.blog-product__head-text'));
        $(element).find('.blog-product__btn-box:not(.blog-product__btn-box--no-js)').insertAfter($(element).find('.blog-product__body'));
      });
      $('.cart__item').each(function (index, element) {
        $(element).find('.cart__item-count').insertAfter($(element).find('.cart__item-cell'));
      });
      if ($('.product-page__controls').length) {
        $('body').removeClass('pb');
      }
      if ($('.checkout__pay-btn-box').length) {
        $('body').removeClass('pb2');
      }
    }
    if ($(window).width() < 1200) {
      $('.product-page__title').insertBefore($('.product-page__mobile-row'));
      $('.product-page__mobile-row').append($('.product-page__price'));
      $('.product-page__price').append($('.js-product-rate'));
      $('.product-page__mobile-head').append($('.product-page__price-descriptions'));
    } else {
      $('.product-page__title').insertAfter($('.product-page__info-head'));
      $('.product-page__controls-left').append($('.product-page__price'));
      $('.product-page__controls-left').append($('.product-page__price-descriptions'));
      $('.product-page__info-head').append($('.js-product-rate'));
    }
  });

  if ($(window).width() < 1200) {
    if ($('.shop__text-btn').length) {
      $('.shop__text-btn').on('click', function () {
        $('.shop__text').toggleClass('active');
      });
    }
    $('.js-mobile-filter').on('click', function () {
      $('.shop__filter').addClass('active');
      setTimeout(function () {
        $('.shop__filter-box').addClass('active');
      }, 300);
    });
    $('.js-close-filter').on('click', function () {
      $('.shop__filter-box').removeClass('active');
      setTimeout(function () {
    ...
    $('.js-mobile-btn').on('click', function () {
      ...
      setTimeout(function () {
        $('.header__middle-box').addClass('active');
      }, 300);
    });
    $('.js-close-mobile-menu').on('click', function () {
      $('.header__middle-box').removeClass('active');
      setTimeout(function () {
  ...
  $('.header__basket').on('click', function (event) {
    event.stopPropagation();
    $('.header__popup').toggleClass('active');
  });

  $('.header__popup-close').on('click', function (event) {
    event.stopPropagation();
  ...
  $(document).on("click touchend", function (event) {
    event.stopPropagation();
    if ($(event.target).closest(".header__popup-box").length) return;
    $(".header__popup").removeClass('active');
  });
  ...
  $(document).on("click touchend", function (event) {
    event.stopPropagation();
    if ($(event.target).closest(".header__search-form").length) return;
...
var _nouislider = __webpack_require__(30);
...
exports.default = function () {
  if ($('#filter-slider').length) {
    ...
    inputIncrement.addEventListener('change', function () {
      slider.noUiSlider.set([null, this.value]);
    });
    inputDecrement.addEventListener('change', function () {
...
var __WEBPACK_AMD_DEFINE_FACTORY__, __WEBPACK_AMD_DEFINE_ARRAY__, __WEBPACK_AMD_DEFINE_RESULT__;/*! nouislider - 14.6.1 - 8/17/2020 */
(function(factory) {
    if (true) {
        // AMD. Register as an anonymous module.
